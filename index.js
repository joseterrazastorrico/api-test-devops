...
app.post('/chat', async (req, res) => {
  ...
  if (!pregunta || typeof pregunta !== 'string' || pregunta.trim() === '') {
    return res.status(400).json({ error: 'La propiedad pregunta es obligatoria y no debe estar vacía' });
  }
  ...
  if (!process.env.OPENAI_API_KEY) {
    return res.status(500).json({ error: 'No se ha configurado la API key de OpenAI' });
  }
